...
### נוכחות נווה ירק — ג-ו (תוקן 30/06/2026)
**הבעיה:** בוט בdot.py שימש בשמות קבוצה `"ג"` ו-`"א"` במקום `"ג-ו"` ו-`"א-ב"`, וגם שעות שגויות — לא התאים ל-`BRANCH_GROUPS` ב-`attendance.py`.

**מיקום:** `bot.py` — dict שעות belt wizard (סביב שורה 4336)
...
("נווה ירק","א-ב", "שלישי"): "17:45",
```
גם שעת גנים תוקנה מ-16:45 ל-16:00.

**לקח:** בכל הוספת קבוצה חדשה — לוודא שהשם זהה בדיוק בין `attendance.py → BRANCH_GROUPS` לבין כל ה-dicts ב-`bot.py`.
...
### נוכחות נווה ירק — ג-ו (תוקן 30/06/2026)
**הבעיה:** bot.py השתמש בשמות קבוצה "ג" ו-"א" במקום "ג-ו" ו-"א-ב", וגם שעות שגויות — לא התאים ל-BRANCH_GROUPS ב-attendance.py.

**מיקום:** bot.py — dict שעות belt wizard (סביב שורה 4336)

**התיקון:**
- "ג" → "ג-ו", שעה 17:45 → 16:45
- "א" → "א-ב", שעה 18:30 → 17:45
- גנים: שעה 16:45 → 16:00

**לקח:** בכל הוספת קבוצה — לוודא שהשם זהה בדיוק בין attendance.py (BRANCH_GROUPS) לבין כל ה-dicts ב-bot.py.
